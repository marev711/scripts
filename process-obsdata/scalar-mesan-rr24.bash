...
root_rr24_grib_folder=/nobackup/smhid9/
output=/nobackup/rossby17/sm_maeva/Data/Primavera-upload/mesan/input/intermediate/rr24/
...
if [ $year -lt 1989 -o $year -gt 2010 ]
then
    root_rr24_grib_folder=${root_rr24_grib_folder}/sm_davhi/euro4m/mesan_e4m/rr24nQC/
else
    root_rr24_grib_folder=${root_rr24_grib_folder}/sm_tland/euro4m/mesan_e4m/rr24nQC/
fi

root_rr24_grib_folder=${root_rr24_grib_folder}/${year}
output=${output}/${year}
mkdir -p ${output}

grid_description_file=/tmp/grid_description-${year}.txt

abs_value_files=$(ls ${root_rr24_grib_folder}/*[^d].grb)
#std_value_files=$(ls ${root_rr24_grib_folder}/????????/*sd.grb)
...
do
    ...
    md5grid=$1
    weightfile=${output}/${input_grib_file%*_[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9].grb}.nc"-weight-"${md5grid}"-"${year}".nc"
